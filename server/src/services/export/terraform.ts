...
export interface TerraformBundle {
  ...
  "variables.tf": string;
  "outputs.tf": string;
  "system-synthesis.manifest.json": string;
}
...
export function generateTerraform(
  ...
  const ir = buildInfrastructureIR(nodes, edges, "terraform");
  const nodeById = new Map(nodes.map((node) => [node.id, node]));
  const orderedNodes = ir.resources.map((resource) => nodeById.get(resource.sourceNodeId)!);
  const nameMap = new Map(ir.resources.map((resource) => [resource.sourceNodeId, resource.name]));

  // ── main.tf ──
  const resources: string[] = [];

  resources.push(`${provenanceHeader(ir)}
...
`);
  ...
  // ── variables.tf ──
  const variables = `${provenanceHeader(ir)}
...
`;
  ...
  return {
    ...
    "system-synthesis.manifest.json": `${JSON.stringify(ir, null, 2)}\n`,
  };
}
