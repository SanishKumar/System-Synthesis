...
Current automated count: 147 architecture-core tests, 13 CLI tests, 24 Action tests, 9 frontend tests, and 277 backend tests (470 total). A further 11 backend tests run the synchronization and decision-audit contracts against a real PostgreSQL; CI provides one and fails if they skip, and locally they are skipped unless `TEST_DATABASE_URL` points at a scratch database. The Next.js production build type-checks and prerenders the review list, review detail, and repository-connections routes.
