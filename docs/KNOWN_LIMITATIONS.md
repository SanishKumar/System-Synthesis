...
- The analyzer identity covers rule behaviour over a stored graph. It does not version the import adapter, so improving Compose extraction does not mark existing reviews outdated; their stored verdicts remain reproducible from their stored graphs, but those graphs may no longer match what the current adapter would extract from the same source. Only a new delivery or a fresh import updates the graph.
