...
- A review stored by an earlier analyzer is reported as outdated but is not recomputed automatically. There is no bulk re-analysis endpoint; a pull-request review refreshes on the next workflow run, and a manual review must be imported again. The analyzer fingerprint also cannot see a rule whose findings change while its id and severity stay the same — that case depends on `ANALYZER_VERSION` being bumped deliberately.
