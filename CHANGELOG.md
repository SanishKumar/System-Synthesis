...
- `docs/REPOSITORY_GLOBAL_IDENTITY_DESIGN.md`, a proposed and deliberately unimplemented design for repository-global integration and review identity. Two verified admins of one repository currently produce two reviews and two check runs under the same required-check name, able to carry opposite conclusions; the document records that reproduction, the current and proposed database keys, rename and transfer behaviour, duplicate detection that refuses to pick a winner, migration ordering and rollback, memory/PostgreSQL parity, an adversarial test matrix, and four open questions. Nothing in it is built
