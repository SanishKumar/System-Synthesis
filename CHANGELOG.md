...
- `docs/REPOSITORY_GLOBAL_IDENTITY_DESIGN.md`, a proposed and deliberately unimplemented design for repository-global integration and review identity. Two verified admins of one repository currently produce two reviews and two check runs under the same required-check name, able to carry opposite conclusions; the document records that reproduction, the current and proposed database keys, one repository-scoped credential rather than one per administrator, tombstoned canonical reviews so a check identity stays stable, collaborator read authorization kept separate from the connector's admin proof, a repository-level audit model with compare-and-set rotation, rename and transfer behaviour including paused ingestion at a new owner, duplicate reconciliation that merges only under strict semantic equivalence and never selects between credential hashes, cleanup of already-published superseded check runs, unresolved repository states that distinguish transient failures from durable ones, twelve migration phases with corrected rollback claims, memory/PostgreSQL parity, an adversarial test matrix, and a discovery note recording that OAuth tokens are discarded. Nothing in it is built
